...
public class ServiceBus : IServiceBus, IEventBus
{
    ...
    {
        var commandConsumer = _serviceProvider.GetService<ICommandConsumer<T>>();
        if (commandConsumer == null)
            throw new ArgumentNullException($"No CommandConsumer was registered for {typeof(T).FullName}");

        await commandConsumer.ConsumeAsync(command, cancellationToken);
    }
    ...
    public async Task PublishAsyncCore<T>(T @event, CancellationToken cancellationToken = default) where T : class, IEvent
    {
        ...
        {
            var domainEvent = (IDomainEvent)@event;
            await PublishDomainEventAsyncCore(domainEvent, cancellationToken);
        }

        if (@event.GetType() == typeof(IIntegrationEvent))
        {
            var integrationEvent = (IIntegrationEvent)@event;
            await PublishIntegrationEventAsyncCore(integrationEvent, cancellationToken);
        }
    }
